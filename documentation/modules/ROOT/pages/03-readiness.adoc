...
[#create]
== Configure Readiness to read from logs inside a container.

This page shows how to configure readiness probes for containers.
...
In this step, you have to be able to deploy these _Jump App_ in order to configure a readiness probe against the container logs. In case of error the container will be restarted.
...
[source,yaml]
----
include::../../../yaml/Readyness/1-build-config.yml[]
----
